...
def _persist(
    ...
    **extra: Any,
    ...
    return persist_assessment_outcome_async(
...
@pytest.fixture
def repository() -> PostgresAssessmentRepository:
    assert DATABASE_URL is not None
    repo = asyncio.run(_connect())
...
def test_migration_creates_five_tables_with_rls_and_no_public_policies(
    repository: PostgresAssessmentRepository,
) -> None:
    async def body() -> None:
        assert DATABASE_URL is not None
...
def test_persist_completed_review_required_not_scorable_and_inaccessible_link(
    repository: PostgresAssessmentRepository,
) -> None:
    se_lines = [
        "Summary",
    ...
    se_result = asyncio.run(_persist(se, se_bytes, repository))
    assert se_result["state"] == "PERSISTED"
    loaded = asyncio.run(repository.get_latest_run(se["assessment_id"]))
    ...
    meta = asyncio.run(repository.get_assessment(se["assessment_id"]))
    ...
    da_result = asyncio.run(_persist(da, da_bytes, repository, assessment_input=da_input))
    ...
    review_result = asyncio.run(
        _persist(review, review_bytes, repository, assessment_input=review_input)
    )
    assert review_result["state"] == "PERSISTED"
    review_run = asyncio.run(repository.get_run("run-review"))
    ...
    unreadable = run_assessment_pipeline(
        ...
        retrieve_link=lambda *_args, **_kwargs: (_ for _ in ()).throw(AssertionError("network")),
    )
    assert unreadable["state"] == "NOT_SCORABLE"
    unreadable_result = asyncio.run(
        _persist(unreadable, unreadable_bytes, repository, assessment_input=payload)
    )
    ...
    link_result = asyncio.run(
        _persist(inaccessible, link_bytes, repository, assessment_input=link_input)
    )
    assert link_result["state"] == "PERSISTED"
    link_run = asyncio.run(repository.get_run("run-link"))
...
def test_idempotency_conflict_immutability_and_cascade(
    repository: PostgresAssessmentRepository,
) -> None:
    lines = ["Junior Software Engineer"]
    file_bytes = _pdf(lines)
    outcome = _run("software_engineering", lines)
    first = asyncio.run(_persist(outcome, file_bytes, repository))
    assert first["state"] == "PERSISTED"
    second = asyncio.run(_persist(outcome, file_bytes, repository))
    ...
    conflict = asyncio.run(_persist(changed, file_bytes, repository))
    assert conflict["state"] == "PERSISTENCE_CONFLICT"
    original = asyncio.run(repository.get_run(outcome["run_id"]))
    assert original is not None
    assert original.assessment_result == outcome["assessment_result"]

    other_track = deepcopy(outcome)
    other_track["assessment_id"] = "assessment-other"
    other_track["run_id"] = "run-other"
    da_on_se = deepcopy(outcome)
    da_on_se["track"] = "data_analytics"
    da_conflict = asyncio.run(
...
def test_transaction_rollback_and_duplicate_children(
    repository: PostgresAssessmentRepository,
) -> None:
    lines = ["Junior Software Engineer"]
    file_bytes = _pdf(lines)
    ...
    result = asyncio.run(_persist(broken, file_bytes, repository, assessment_input=broken_input))
    assert result["state"] == "PERSISTENCE_FAILED"
    assert asyncio.run(repository.get_assessment("assessment-rollback")) is None
    assert asyncio.run(repository.get_run("run-rollback")) is None
    ...
    dup_result = asyncio.run(
        _persist(dup, file_bytes, repository, assessment_input=_input(outcome["track"], file_bytes))
    )
    assert dup_result["state"] == "PERSISTENCE_FAILED"
    assert asyncio.run(repository.get_run("run-dup")) is None
    ...
    if facts["evidence_facts"]:
        ...
        facts_result = asyncio.run(
            _persist(
                facts, file_bytes, repository, assessment_input=_input(outcome["track"], file_bytes)
            )
        )
        assert facts_result["state"] == "PERSISTENCE_FAILED"
        assert asyncio.run(repository.get_run("run-facts")) is None


def test_claim_token_and_expires_at_foundations(
    repository: PostgresAssessmentRepository,
) -> None:
    lines = ["Junior Software Engineer"]
    file_bytes = _pdf(lines)
    ...
    meta = asyncio.run(repository.get_assessment(outcome["assessment_id"]))
    assert meta is not None
